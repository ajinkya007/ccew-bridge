...
public class SavedService {
	...
	public void tosave(SavedStock s) {
		...
		DBCollection coll = mongoService.getDb().getCollection("Saved");
	...
	public ArrayList<SavedStock> toretrieve(String usr) {
		...
		cursor1 = mongoService.getDb().getCollection("Saved").find(searchquery).sort(new BasicDBObject("date", -1)).sort(new BasicDBObject("time", -1));
